...
class File
{
    ...
    */
    ...
    //文件是否存在，区分文件大小写
    public static  function file_exist_case($fileName)
    {
    ...
    {
        if (!$dh = @opendir($dir)) {
            return array('file_num'=>0,'folder_num'=>0,'size'=>0);
        }
    ...
    */
    ...
    // 安全读取文件，避免并发下读取数据为空
    public static  function file_read_safe($file, $timeout = 0.1)
    {
    ...
    // 安全读取文件，避免并发下读取数据为空
    public static  function file_wirte_safe($file, $buffer, $timeout=0.1)
    {
     ...
     */
    public static  function file_put_out($file, $download=false, $download_filename=false)
    {
        $error = false;
        if (!file_exists($file)) {
            $error = 'file not exists';
        } elseif (!path_readable($file)) {
            $error = 'file not readable';
        } elseif (!$fp = @fopen($file, "rb")) {
     ...
     */
    public static  function file_download_this($from, $file_name, $header_size=0)
    {
        @set_time_limit(0);
    ...
    public static  function kod_move_uploaded_file($from_path, $save_path)
    {
     ...
     * upload('file','D:/www/');
     */
    public static  function upload_chunk($fileInput, $path = './', $temp_path, $repeat_action)
    {
        global $in;
